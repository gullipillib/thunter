    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (strResponse != "")
        {
            ...
            if (line == "SUCCESS")
            {
                ...
                {
                    AccessDataSource1.SelectCommand = "SELECT * FROM tspots where tsowner ='" + Convert.ToString(Session["loggeduser"]) + "' order by tsorder DESC";
                    ...
                    AccessDataSource1.UpdateCommand = "UPDATE loggedusers SET luspots = '" + Convert.ToString(Convert.ToInt16(lutspots) + 1) + "' where luname='" + Convert.ToString(Session["loggeduser"]) + "'";
	...
	if (tsowner != "TresureHunter")
{
                    ...
                    AccessDataSource1.Update();
                    AccessDataSource1.SelectCommand = "SELECT lutspots,luname FROM loggedusers where luname='" + Convert.ToString(Session["loggeduser"]) + "'" ;
                    dv = (DataView)AccessDataSource1.Select(DataSourceSelectArguments.Empty);
                    dt = new DataTable();
                    dt = dv.ToTable();
                    uniname = dt.DefaultView;
                    string lutspots = dt.Rows[0].Field<string>("lutspots"); //usethis to get field value
                    AccessDataSource1.UpdateCommand = "UPDATE loggedusers SET lutspots = '" + Convert.ToString(Convert.ToInt16(lutspots) + 1) + "' where luname='" + Convert.ToString(Session["loggeduser"]) + "'";
                    AccessDataSource1.Update();
                    ...
                    AccessDataSource1.InsertCommand = "INSERT INTO payments(anount,uname) Values ('" + Convert.ToString(Convert.ToInt16(results["payment_gross"]) - 5 / 2) + "','" + Convert.ToString(Session["loggeduser"]) + "')";
                    AccessDataSource1.Insert();
...
if (tsowner == "TresureHunter")
{
                ...
                {
                    AccessDataSource1.SelectCommand = "SELECT * FROM toolbox  where tbowner =' " + Convert.ToString(Session["loggeduser"]) + "' order by tborder DESC";
                    DataView dv = (DataView)AccessDataSource1.Select(DataSourceSelectArguments.Empty);
